...
if __name__ == "__main__":
    ...
    source = '/home/zhenyi/Project/nnUNet/nnUNet_REFUGE/REFUGE'

    dataset_name = 'Dataset001_REFUGE'
    ...
    with multiprocessing.get_context("spawn").Pool(8) as p:
        ...
        valid_ids = subfiles(join(train_source, 'masks'), join=False, suffix='bmp')
        num_train = len(valid_ids)
        r = []
        for v in valid_ids:
            image_filename = v[:-4] + '.jpg'
            r.append(
                p.starmap_async(
        ...
        # test set
        valid_ids = subfiles(join(test_source, 'masks'), join=False, suffix='bmp')
        for v in valid_ids:
            image_filename = v[:-4] + '.jpg'
            r.append(
                p.starmap_async(
    ...
    generate_dataset_json(join(nnUNet_raw, dataset_name), {0: 'R', 1: 'G', 2: 'B'}, {'background': 0, 'od': 1, 'oc': 2},
